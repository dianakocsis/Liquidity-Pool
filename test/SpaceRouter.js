...
describe('SpaceRouter.sol', function () {
  ...
  describe('Adding liquidity', function () {
    it('first adding of liquidity', async function () {
      ...
      expect(await spaceCoin.balanceOf(pool.address)).to.equal(
        '150000000000000000000'
      );

      // space ico's balance of lp tokens
      expect(await pool.balanceOf(spaceICO.address)).to.equal(
        '67082039324993689892'
      );
    ...
    it('second adding of liquidity', async function () {
      ...
      await sr.addLiquidity(
        spaceCoin.address,
        spaceCoin.balanceOf(owner.address),
        owner.address,
        { value: parseEther('30') }
      );
      ...
      expect(await pool.balanceOf(owner.address)).to.equal(
        '67082039324993689892'
      );
  ...
  describe('Swapping', function () {
    it('Swapping tokens for eth', async function () {
      ...
      await sr.swapExactTokensForETH(
        spaceCoin.address,
        '5000000000000000000',
        owner.address
      );
    ...
    it('Swapping eth for tokens', async function () {
      ...
      await sr.swapExactETHForTokens(spaceCoin.address, owner.address, {
        value: parseEther('1'),
      });
    });
  });
